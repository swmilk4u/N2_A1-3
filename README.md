...
* **실시간 Webhook 동작 메커니즘**:
  ...
  - 사용자가 AI 코칭 피드백 제출에 성공하면, 백엔드 서버에서 환경 변수 **`AUTO_WEBHOOK_URL`**에 등록된 외부 도구(예: Make.com, Zapier의 Custom Webhook 또는 Discord/Slack 인커밍 웹훅) 주소로 사용자의 입력값(직무, 스택, 경험 명세)과 생성된 AI 분석 요약 JSON 데이터를 실시간 POST 전송합니다.
  - 이를 통해 '구직자 이력서 분석 신청(입력) -> AI 분석 수행(처리) -> 외부 디스코드 메신저 실시간 알림 발송 및 구글 시트 저장 연동(저장/알림)' 비즈니스 운영 파이프라인이 정상 동작합니다. 환경 변수 미구성 시에는 에러 없이 패스되도록 설계하여 메인 서비스의 결합도를 낮췄습니다.
